...
/* Green top accent bar matching slippi.gg nav */
[data-testid="stHeader"] { background-color: #2d0a4e; }

/* Make the sidebar collapse/expand toggle clearly visible */
...
/* Tighten up padding on smaller viewports */
...
with st.sidebar:
    ...
    if st.button("📁 Browse for Replay Folder", use_container_width=True):
        root = tk.Tk()
        root.withdraw()
    ...
    st.markdown("---")
    scan_btn   = st.button("📂 Scan Replays", use_container_width=True)
...
m2.markdown(_stat_card("Win Rate",    f"{win_pct:.1f}%",                                color=WIN_COLOR), unsafe_allow_html=True)
m3.markdown(_stat_card("Wins",        str(total_wins),                                  color=WIN_COLOR), unsafe_allow_html=True)
m4.markdown(_stat_card("Losses",      str(total_losses),                                color="#e74c3c"), unsafe_allow_html=True)
...
if not games_df.empty and "match_id" in games_df.columns:
    ...
    with _sel_col:
        form_n = st.selectbox("", [5, 10, 25], index=1, key="recent_form_n", label_visibility="collapsed")
...
def build_matchup_chart() -> go.Figure | None:
    if games_df.empty:
        return None

    matchup = (
        games_df.groupby("char_name")
    ...
    fig.update_layout(
        barmode="stack",
        title="Winrate by Matchup (min. 3 games)",
...
def build_player_char_chart() -> go.Figure | None:
    ...
    fig.update_layout(
        barmode="stack",
        title="Your Win % by Character Played (ranked)",
        xaxis=dict(title="", range=[0, 130]),
        yaxis_title="",
        template=THEME,
        paper_bgcolor=PAPER,
...
def build_sets_df() -> pd.DataFrame:
    if games_df.empty or "match_id" not in games_df.columns:
        return pd.DataFrame()
...
tab_last, tab_gameplay, tab_deep, tab_rating = st.tabs(["⚡ Last Session", "🎮 Gameplay Stats", "🔬 Deep Stats", "Rating Progression"])

with tab_last:
    # -----------------------------------------------------------------------
    # Last Session tab — detailed breakdown of the most recent play session
    # -----------------------------------------------------------------------
    def _get_last_session() -> pd.DataFrame:
        """Return games from the most recent session (2-hour gap groups)."""
        if games_df.empty:
            return pd.DataFrame()
        df = games_df.sort_values("timestamp").copy()
        df["gap"] = df["timestamp"].diff()
    ...
    if last_sess.empty:
        ...
        # Count complete sets in the session
        sess_sets_count = sum(
            1 for mid, sg in last_sess.groupby("match_id")
            if mid and max(int(sg["won"].sum()), len(sg) - int(sg["won"].sum())) >= 2
        )

        # ---- Session header stat cards ----
...
<div style="background:#2b2b2b;border-radius:10px;padding:14px 12px;text-align:center;border:1px solid #3a3a3a;border-top:3px solid {res_color};">
  ...
  <div style="font-size:0.75rem;color:{res_color};margin-top:2px;">{s["result"]}</div>
        ...
        # ---- Stage performance this session ----
        st.markdown("---")
        st.markdown("##### Stages")
        sess_stage = (
        ...
        if not last_sess.empty:
            ...
            fig_mom.add_trace(go.Scatter(
                x=ordered_sess["game_num"],
                y=ordered_sess["cumulative"],
                mode="lines+markers",
                line=dict(color=WIN_COLOR, width=2.5),
                marker=dict(
                    size=10,
...
with tab_rating:
    ...
    if winrate_fig:
        st.plotly_chart(winrate_fig, use_container_width=True)
    else:
...
with tab_gameplay:
    ...
    with col_left:
        matchup_fig = build_matchup_chart()
        if matchup_fig:
    ...
    # -----------------------------------------------------------------------
    # Replay lookup by opponent character
    # -----------------------------------------------------------------------
    ...
    if not games_df.empty:
        ...
        if selected_char != "— select a character —":
            ...
            # Convert frames to MM:SS (Melee runs at 60 fps)
            ...
            styled = display_df.style.applymap(_color_result, subset=["Result"])
            st.dataframe(styled, hide_index=True, use_container_width=True)
    else:
        st.info("Scan replays to enable replay lookup.")

    st.markdown("---")

    player_char_fig = build_player_char_chart()
    if player_char_fig:
        st.plotly_chart(player_char_fig, use_container_width=True)
    else:
        st.info("Character performance data appears after ranked replays are scanned.")

    st.markdown("---")

    st.subheader("Recent Sets")
    sets_df = build_sets_df()
    if not sets_df.empty:
        def color_result(val):
            color = WIN_COLOR if val == "Win" else "#e74c3c"
            return f"color: {color}; font-weight: bold"

        styled = sets_df.style.applymap(color_result, subset=["Result"])
        st.dataframe(styled, hide_index=True, use_container_width=True)
    else:
...
with tab_deep:
    ...
    # Clutch Stats
    # -------------------------------------------------------------------
    st.subheader("Clutch Stats")

    # Game Win Rate and Set Win Rate overview cards
    if not games_df.empty:
        ...
        ov2.markdown(_stat_card(
            ...
            f"{set_wins_count}W / {set_total_count - set_wins_count}L" if set_total_count > 0 else "No complete sets",
    ...
    if not sessions_df.empty:
        def _color_session(val):
            ...
            if val == "Negative":
                return "color: #e74c3c; font-weight: bold"
            return "color: #aaa"
        styled_sessions = sessions_df.style.applymap(_color_session, subset=["Result"])
        st.dataframe(styled_sessions, hide_index=True, use_container_width=True)
    else:
    ...
    if tod_fig:
        ...
        st.info("Time-of-day chart appears after more games are scanned (need at least 3 games in a single hour).")
    ...
    if not games_df.empty:
        ...
        # Summary table: all opponents with W/L record
        opp_summary = (
        ...
        # Count complete sets per opponent
        set_counts: dict = {}
        for mid, sg in games_df.groupby("match_id"):
            if not mid:
                continue
        ...
        opp_summary = opp_summary.sort_values("total", ascending=False).rename(columns={
            "opponent_code": "Opponent", "wins": "W", "losses": "L",
            "total": "Games", "win_pct": "Win %", "sets": "Sets",
        })[["Opponent", "Sets", "Games", "W", "L", "Win %"]]
        ...
        styled_opp = opp_summary.style\
            .format({"Win %": "{:.1f}%"})\
            .applymap(_color_winpct, subset=["Win %"])
        st.dataframe(styled_opp, hide_index=True, use_container_width=True)

        st.markdown("##### Set-by-set breakdown")
        selected_opp = st.selectbox(
            "Select opponent:",
            ["— select —"] + all_opponents,
            key="opp_history_select",
        )
        if selected_opp != "— select —":
            ...
            if not opp_sets.empty:
                ...
                styled_sets = opp_sets.style.applymap(_color_set_result, subset=["Result"])
                st.dataframe(styled_sets, hide_index=True, use_container_width=True)
    else:
